# GraphGaitODE: Neuromotor Graph Network for Gait Kinematics
...
Requires Python 3.11+. We recommend [`uv`](https://docs.astral.sh/uv/) for environment management.
...
We provide a template script to read your CSVs, segment the data by heel strikes, interpolate/normalize to 101 points (0-100% of gait cycle), and perform the **critical canonicalization step** (swapping left/right so that the reference/striking limb is always in the same column position).
